...
# --- Seitenkonfiguration ---
st.set_page_config(page_title="Interaktiver Zweifeldträger", layout="wide")
st.title("Statik-Tool: Zweifeldträger mit Kragarmen")

def berechne_zweifeldtraeger(L_k1, L1, L2, L_k2, q_k1, q1, q2, q_k2, F, x_F):
    x_A = L_k1
    x_B = L_k1 + L1
    x_C = L_k1 + L1 + L2
    ...
    def macaulay(x_vals, a, power):
        if power == 0:
            return np.where(x_vals >= a, 1.0, 0.0)
        else:
            return np.where(x_vals > a, (x_vals - a)**power, 0.0)
    ...
    M0_load = (get_load_M(x, q_k1, 0, L_k1) + 
               get_load_M(x, q1, L_k1, x_B) + 
               get_load_M(x, q2, x_B, x_C) + 
               get_load_M(x, q_k2, x_C, L_tot))
    ...
    V0_load = (get_load_V(x, q_k1, 0, L_k1) + 
               get_load_V(x, q1, L_k1, x_B) + 
               get_load_V(x, q2, x_B, x_C) + 
               get_load_V(x, q_k2, x_C, L_tot))
    ...
    return x, V, M, A_y, B_y, C_y, L_tot, x_A, x_B, x_C

# --- Eingabemaske in der Seitenleiste (Sidebar) ---
...
L2   = st.sidebar.number_input("Feld 2", min_value=0.1, value=4.0, step=0.5)
...
q2   = st.sidebar.number_input("Last Feld 2", value=8.0, step=1.0)
...
L_tot_max = L_k1 + L1 + L2 + L_k2
x_F = st.sidebar.number_input("Position von F (von links)", min_value=0.0, max_value=float(L_tot_max), value=3.5, step=0.5)

# --- Berechnung ---
x, V, M, A_y, B_y, C_y, L_tot, x_A, x_B, x_C = berechne_zweifeldtraeger(L_k1, L1, L2, L_k2, q_k1, q1, q2, q_k2, F, x_F)

# --- Ausgabe der Auflagerkräfte ---
...
if F > 0 or F < 0:
    ax1.annotate('', xy=(x_F, 0), xytext=(x_F, 1.2), arrowprops=dict(facecolor='magenta', shrink=0.0, width=2, headwidth=8))
    ax1.text(x_F, 1.3, f"F = {F} kN", ha='center', color='magenta')

ax1.set_xlim(-0.5, L_tot + 0.5)
ax1.set_ylim(-0.5, 2.0)
ax1.axis('off')
ax1.set_title("Statisches System und Belastung", fontweight='bold')
